// Package cmdface provide a bash like interactive interface as front end to test.
//
// You can get user input by Input() and print string by Show().
// The Show() function print string one by one, so don't warry about messages mixed while
// calling concurrent.
//
// You can define a new command by AddCommand() that while add your command to command list.
// InputAndRunCommand() ask user to input a cmd string and find command node in command list
// then call then cmdFunc of the command with parameters.
package cmdface
...
func InputAndRunCommand(prompt string) error {
	inputStr := strings.TrimSpace(Input(prompt))
...
// Input show prompt on the terminal then get string from stdin.
